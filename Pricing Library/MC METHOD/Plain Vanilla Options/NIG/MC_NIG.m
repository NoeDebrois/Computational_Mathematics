function [call_price, call_CI_price, put_price, put_CI_price] = MC_NIG(S0, K, r, T, sigma, theta, kNIG, M, Nsim, flag, Nsim2)
...
% M     : number of time steps betwenn 0 and T
...
if flag == 0 % NO VARIANCE REDUCTION
    [~, ST] = NIG_ASSET(S0, r, T, sigma, theta, kNIG, M, Nsim);
    DiscPayoffs = exp(-r * T) * max(ST - K, 0); % Call

elseif flag == 1 % ANTITHETIC VARIABLES
    [~, ~, ST, STAV] = NIG_ASSET_AV(S0, r, T, sigma, theta, kNIG, M, Nsim);
    Payoff1 = max(ST - K, 0);
...
elseif flag == 2 % CONTROL VARIABLES
    ...
    % -------- f & g -------- %
    f = NIG_ASSET(S0, r, T, sigma, theta, kNIG, M, Nsim2);
    ...
    % 2 - Compute price
    % -------- f & g -------- %
    [~, New_f] = NIG_ASSET(S0, r, T, sigma, theta, kNIG, M, Nsim);
...
params.theta = theta;
params.kNIG = kNIG;
CHECK_CM_CALL_PRICE = NIG_CARR_MADAN(K, params, T, r, S0);
