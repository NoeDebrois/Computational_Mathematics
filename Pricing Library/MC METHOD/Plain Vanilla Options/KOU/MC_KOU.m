function [call_price, call_CI_price, put_price, put_CI_price] = MC_KOU(S0, K, r, T, sigma, p, lambdap, lambdam, lambdaK, M, Nsim, flag, Nsim2)
...
if flag == 0 % NO VARIANCE REDUCTION
    [~, ST] = KOU_ASSET(S0, r, T, sigma, p, lambdap, lambdam, lambdaK, M, Nsim);
    DiscPayoffs = exp(-r * T) * max(ST - K, 0); % Call (you can change the payoff for extic options)

elseif flag == 1 % ANTITHETIC VARIABLES
    [~, ~, ST, STAV] = KOU_ASSET_AV(S0, r, T, sigma, p, lambdap, lambdam, lambdaK, M, Nsim);
    Payoff1 = max(ST - K, 0);
...
elseif flag == 2 % CONTROL VARIABLES
    ...
    % -------- f & g -------- %
    f = KOU_ASSET(S0, r, T, sigma, p, lambdap, lambdam, lambdaK, M, Nsim2);
    ...
    % 2 - Compute price
    % -------- f & g -------- %
    [~, New_f] = KOU_ASSET(S0, r, T, sigma, p, lambdap, lambdam, lambdaK, M, Nsim);
...
params.lambdam = lambdam;
params.lambdaK = lambdaK;
CHECK_CM_CALL_PRICE = KOU_CARR_MADAN(K, params, T, r, S0);
