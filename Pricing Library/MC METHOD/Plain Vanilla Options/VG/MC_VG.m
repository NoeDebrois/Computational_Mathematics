function [call_price, call_CI_price, put_price, put_CI_price] = MC_VG(S0, K, r, T, sigma, theta, kVG, M, Nsim, flag, Nsim2)
...
if flag == 0 % NO VARIANCE REDUCTION
    [~, ST] = VG_ASSET(S0, r, T, sigma, theta, kVG, M, Nsim);
    DiscPayoffs = exp(-r * T) * max(ST - K, 0); % Call. You can change the payoff, example for path dependent : exp(-r * T) * max(S(:, end) - mean(S(:, 2:end), 2), 0)

elseif flag == 1 % ANTITHETIC VARIABLES
    [~, ~, ST, STAV] = VG_ASSET_AV(S0, r, T, sigma, theta, kVG, M, Nsim);
    Payoff1 = max(ST - K, 0);
...
elseif flag == 2 % CONTROL VARIABLES
    ...
    % -------- f & g -------- %
    f = VG_ASSET(S0, r, T, sigma, theta, kVG, M, Nsim2);
    ...
    % 2 - Compute price
    % -------- f & g -------- %
    [~, New_f] = VG_ASSET(S0, r, T, sigma, theta, kVG, M, Nsim);
...
params.theta = theta;
params.kVG = kVG;
CHECK_CM_CALL_PRICE = VG_CARR_MADAN(K, params, T, r, S0);
