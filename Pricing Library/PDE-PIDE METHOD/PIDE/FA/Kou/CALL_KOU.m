% ----------------------------------------------------------------------- %
clear; close all;
% Price an European Call option under the Kou model
...
lambdaK = 2; lambdap = 10; lambdam = 3; p = 0.6;
...
% Lévy Measure "nu" of Kou model:
nu = @(y) p * lambdaK * lambdap * exp(-lambdap * y) .* (y > 0) + ...
    (1 - p) * lambdaK * lambdam * exp(-lambdam * abs(y)) .* (y < 0);
...
price_cm = FFT_CM_Call_Kou(K, [sigma, lambdaK, p, lambdap, lambdam], T, r, S0) % CALL !
